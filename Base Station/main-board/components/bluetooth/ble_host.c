...
#define TEST_DEVICE_NAME            "Team_22_BLE"
...
struct gatts_profile_inst { // likely will be very helpful for own development
...
esp_err_t ble_init(void) {
    ...
    esp_status = esp_ble_gatts_register_callback(gatts_event_handler);
    if (esp_status){
        ESP_LOGE(GATTS_TAG, "gatts register error, error code = %x", esp_status);
        return esp_status;
    }
    esp_status = esp_ble_gap_register_callback(gap_event_handler);
    if (esp_status){
        ESP_LOGE(GATTS_TAG, "gap register error, error code = %x", esp_status);
        return esp_status;
    }
    esp_status = esp_ble_gatts_app_register(PROFILE_A_APP_ID);
    if (esp_status){
        ESP_LOGE(GATTS_TAG, "gatts app register error, error code = %x", esp_status);
        return esp_status;
    }
